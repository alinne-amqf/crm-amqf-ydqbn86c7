...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseAdmin
      .from('profiles')
      .select('status, email, id, has_accessed, name')
      .eq('id', user_id)
      .single()
    ...
    const emailHtml = `Oi ${userName}, voce foi convidado para acessar o CRM AMQF. Clique aqui para criar sua senha: ${appUrl}/convite?token=${token}. Link valido por 7 dias.`
